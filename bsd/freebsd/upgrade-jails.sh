...
for JAIL_PATH in /usr/local/bastille/jails/*; do
    JAIL=$(basename $JAIL_PATH) && echo -n "-- $JAIL: " \
        && grep osrelease $JAIL_PATH/jail.conf >/dev/null 2>/dev/null ; \
        if [ $? -eq 0 ]; then echo "Thin jail" ; else echo "Thick jail"; fi ;
done
